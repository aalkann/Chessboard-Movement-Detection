...
public:
    ...
    void apply_changes(Mat bird_eye_frame, int radius) {
        ...
        putText(bird_eye_frame, "Old " + std::to_string(movement_old_index), Point(mover_old_position.x - 10, mover_old_position.y - 20), FONT_HERSHEY_SIMPLEX, 1, Scalar(255, 0, 0), 2);
        circle(bird_eye_frame, mover_old_position, radius, Scalar(255, 0, 0), 3);

        putText(bird_eye_frame, "New " + std::to_string(movement_new_index), Point(mover_new_position.x - 10, mover_new_position.y - 20), FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 255, 0), 2);
        ...
        if (!defeated_indexes.empty()) {
            ...
            for (int i = 0; i < defeated_indexes.size(); i++) {
                putText(bird_eye_frame, "Defeated " + std::to_string(defeated_indexes[i]), Point(defeated_positions[i].x - 10, defeated_positions[i].y - 20), FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 0, 255), 2);
